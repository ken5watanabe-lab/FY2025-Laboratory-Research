...
def process(data, labels, name, params, verbose=0):
    ...
    save_fft_npz(
        [emg_entropy],
        params={"merge": params['EMG_ENTROPY_MERGE']},
        epoch_size=params['EPOCH_SIZE'],
        out_path=Path(DATA_FOLDER, "spectral_band", name + "_emg_entropy.npz"),
        ch_names=[f"emg_entropy_{i}" for i in range(emg_entropy.data.shape[1])],
        extra_meta={"source": name}
    )
